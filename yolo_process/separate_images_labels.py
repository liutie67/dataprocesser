...
def separate_images_labels(path='', log=False):
    """
    ...
    Example:
        >>> # 基本用法
        >>> separate_images_labels('./dataset')
        >>>
        >>> # 启用日志记录
        >>> separate_images_labels('./dataset', log=True)
        >>>
        >>> # 处理当前目录
        >>> separate_images_labels()
    """
    # 配置日志
    if log:
        logging.basicConfig(
...
if __name__ == "__main__":
    ...
    separate_images_labels(path, log=False)
